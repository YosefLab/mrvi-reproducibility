...
pdata
# %%
dc.plot_filter_by_expr(pdata, group="donor_status", min_count=20, min_total_count=15)
# %%
genes = dc.filter_by_expr(pdata, group="donor_status", min_count=20, min_total_count=15)
...
dds = DeseqDataSet(
    ...
    ref_level=["donor_status", vehicle_cluster],
    refit_cooks=True,
    n_cpus=8,
)
dds.deseq2()
...
# %%
compare_cluster = "Cluster 6"
stat_res = DeseqStats(
    dds, contrast=["donor_status", compare_cluster, vehicle_cluster], n_cpus=8
)
stat_res.summary()
stat_res.lfc_shrink(coeff=f"donor_status_{compare_cluster}_vs_{vehicle_cluster}")
results_df = stat_res.results_df

# %%
dc.plot_volcano_df(
    results_df[np.abs(results_df["log2FoldChange"]) >= 0.5],
    x="log2FoldChange",
    y="padj",
    top=20,
)
