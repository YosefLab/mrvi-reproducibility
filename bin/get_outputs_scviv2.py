...
def get_outputs_scviv2(
    *,
    ...
    cell_specific_dists = xr.open_dataarray(cell_distance_matrices_in)
    cell_specific_normalized_dists = xr.open_dataarray(
        cell_normalized_distance_matrices_in
    )
    ...
    for group in groups:
        group_mask = (cell_groups == group).to_numpy()
        dists = cell_specific_dists.sel(obs_name=group_mask).mean("obs_name")
        normalized_dists = cell_specific_normalized_dists.sel(obs_name=group_mask).mean(
            "obs_name"
        )
        all_dists.append(dists.data[None])
        all_normalized_dists.append(normalized_dists.data[None])
