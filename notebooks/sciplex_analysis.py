...
for cl in cell_lines:
    ...
    for phase in dists.phase_name.values:
        ...
        #         bbox_to_anchor=(1.3, 0.9), bbox_transform=plt.gcf().transFigure, loc='upper right')
        # plt.gca().add_artist(product_legend)
        # plt.savefig(os.path.join(figures_path, f"sciplex_{cl}_significant_filtered_phase_{phase}.{method_name}.distance_matrices_heatmap.png"), bbox_inches="tight", dpi=300)
        # plt.clf()
    ...
    vmax = max(np.percentile(dists.cell.values, 95), np.percentile(normalized_dists.cell.values, 95))
    binwidth = 0.1
    bins = np.arange(0, vmax + binwidth, binwidth)
